...
function brmedia_import_text_input_callback($args) {
    $options = get_option('brmedia_import_settings');
    $field = $args['field'];
    $value = isset($options[$field]) ? $options[$field] : '';
    echo '<input type="text" class="form-control" name="brmedia_import_settings[' . esc_attr($field) . ']" id="' . esc_attr($field) . '" value="' . esc_attr($value) . '" placeholder="' . esc_attr($args['placeholder']) . '" />';
}
...
function brmedia_get_default_template_options() {
    return array(
        ...
        'padding' => '10px',
        'margin' => '0 auto',
        'controls' => ['volume', 'speed', 'rewind', 'fast_forward'],
        'enable_marquee' => false,
        'marquee_speed' => 10,
        'marquee_behaviour' => 'scroll',
        'marquee_direction' => 'left',
        'marquee_loop' => -1,
        'enable_tracklist' => false,
