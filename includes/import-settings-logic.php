...
function brmedia_import_enabled_callback($args) {
    ...
    echo '<input type="checkbox" class="toggle-service" name="brmedia_import_settings[' . esc_attr($args['field']) . ']" id="' . esc_attr($args['field']) . '" value="1" ' . checked(1, $value, false) . ' data-service="' . esc_attr($args['key']) . '" /> ' . esc_html($args['label']);
}
